...
from src.sql.sql_port import SqlPort
...
class SqliteAdapter(SqlPort):
    def _find_highest_version_table(self, cursor: sqlite3.Cursor, base_name: str) -> str:
        cursor.execute(settings.SQL_TABLE_MAX_VERSION_QUERY, (f"{base_name}{settings.TABLE_VERSION_SUFFIX}",))
    ...
    def _query_all_from_table(self, db_path: str, base_table_name: str) -> List[Dict[str, Any]]:
        ...
        cursor.execute(settings.SQL_SELECT_ALL_QUERY % table_name)
    ...
    def get_devices(self, db_path: str) -> List[Dict[str, Any]]:
        return self._query_all_from_table(db_path, settings.TABLE_PREFIX_DEVICES)

    def get_endpoints(self, db_path: str) -> List[Dict[str, Any]]:
        return self._query_all_from_table(db_path, settings.TABLE_PREFIX_ENDPOINTS)

    def get_node_descriptors(self, db_path: str) -> List[Dict[str, Any]]:
        return self._query_all_from_table(db_path, settings.TABLE_PREFIX_NODE_DESCRIPTORS)
